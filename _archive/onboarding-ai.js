...
(function () {
  ...
  // Bridged to the app's single key field (S.apiKey). Falls back to legacy S.geminiApiKey.
  function geminiKey() { return (S && (S.apiKey || S.geminiApiKey) ? String(S.apiKey || S.geminiApiKey).trim() : '') || GEMINI_DEV_KEY; }
...
החזר JSON בלבד, ללא טקסט נוסף וללא הסברים.`;
  ...
  async function runExtract() {
    ...
    if (keyInp && keyInp.value.trim()) { S.apiKey = keyInp.value.replace(/\s+/g, ''); save(); }
