...
function deletePlannerCourse(id) {
  ...
  // Snapshot everything this delete removes, so it can be fully restored via undo.
  const snap = {
    course,
    tasks:    S.tasks.filter(t => t.course === course.name),
    exams:    S.exams.filter(e => e.course === course.name),
    homework: (S.homework || []).filter(h => h.course === course.name),
  };
  ...
  if (typeof sfShowUndoToast === 'function') {
    sfShowUndoToast(`הקורס "${course.name}" נמחק`, () => {
      if (!S.courses.some(c => c.id === snap.course.id)) S.courses.push(snap.course);
      S.tasks.push(...snap.tasks);
      S.exams.push(...snap.exams);
      if (!Array.isArray(S.homework)) S.homework = [];
      S.homework.push(...snap.homework);
      save(); renderAll(); renderPlannerPage();
    });
  }
}
