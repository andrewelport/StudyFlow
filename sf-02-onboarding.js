...
function updateHeaderStats() {
  ...
  if (urgent) {
    const daysLeft = Math.max(0, Math.ceil((new Date(urgent.date+'T12:00')-new Date())/86400000));
    if (_ueBanner) _ueBanner.classList.remove('hidden');
    const _ueText = document.getElementById('urgent-exam-text');
    if (_ueText) _ueText.textContent = `מבחן ב-${urgent.course} בעוד ${daysLeft} ימים!`;
