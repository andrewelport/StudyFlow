...
היה ספציפי (לא "תלמד יותר"). דרג סיכון: ירוק=במסלול, צהוב=דורש תשומת לב, אדום=בסיכון.`,
  ...
  function aiError(e) {
    ...
    return `<div style="color:var(--red,#e5484d);font-weight:600">${esc(msg)}</div><div style="color:var(--muted,#888);font-size:.85rem;margin-top:.4rem">צריך מפתח Gemini API — הוסף אותו בהגדרות.</div>`;
  }
  ...
  async function sfCoachSend() {
    ...
    try {
      ...
      thinking.remove();
      S.coachHistory.push({ role: 'assistant', content: (e && e.message ? e.message : 'שגיאה') + ' — בדוק מפתח Gemini בהגדרות.', ts: Date.now() });
      save(); sfRenderCoach();
    }
  }
